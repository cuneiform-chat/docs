...
- [Anexos de documentos](/integrations/document-attachments) — anexe um arquivo e pergunte sobre ele (ativado por padrão, planos pagos)
...
Alguém pode anexar um arquivo a uma mensagem e perguntar ao seu agent sobre ele. **Cada arquivo anexado a uma mensagem é lido**, não só o primeiro. Uma mensagem que leva apenas uma figurinha não tem nada para ler, então o bot responde como já responde hoje.

No Discord isso vem **ativado por padrão**: um admin pode desativar **Permitir anexos de documentos** no card da integração. Veja [Anexos de Documentos](/integrations/document-attachments) para os tipos suportados, os limites e a configuração.
