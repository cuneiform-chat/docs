...
<Callout type="info">
  Esta página cobre o Mini App de loja voltado para o cliente que você pode publicar em qualquer integração do **Meta (Facebook)**. O motor da loja é compartilhado com a [Loja no Telegram](/integrations/telegram-mini-app) — mesmo editor, mesmos blocos, mesmos temas — e uma mesma loja pode ser publicada em uma página do Facebook *e* em um bot do Telegram ao mesmo tempo. Apenas o lançador e alguns comportamentos específicos do Messenger são diferentes.
</Callout>
...
- **Os clientes nunca saem do Messenger.** Sem instalação de aplicativo, sem domínio para lembrar e sem página de checkout separada para construir. A loja abre no WebView do Messenger e herda o tema e o idioma do usuário.
...
- **Gratuito em todos os planos.** Sem limites de páginas, blocos por página ou documentos de produto — em nenhum plano, inclusive no teste gratuito. O preço é por organização — adicionar uma segunda página do Facebook não muda o seu preço.
...
- **Duas formas de obter os produtos.** Uma loja **montada à mão** é uma superfície de catálogo sofisticada + captura de leads — você digita os produtos, e ela não faz pagamentos, nem rastreamento de estoque, nem frete. Uma loja **vinculada ao catálogo** puxa os produtos de uma [loja Shopify ou WooCommerce conectada](/commerce) e leva os compradores ao seu checkout real.
...
<Callout type="info">
  **Duas formas de começar uma loja.** A mais rápida é o hub **Storefront** (menu lateral → **Storefront** → **Criar loja**): ele cria uma loja nova que você pode montar primeiro e publicar em qualquer canal depois — em uma Página do Facebook, um bot do Telegram ou ambos — pela aba **Distribute** dela. A entrada por Página abaixo (**Integrações → sua Página do Facebook → Configurar mini app**) continua funcionando e é prática quando você já está no card da Página.
</Callout>
...
O mesmo escopo da Loja no Telegram. Estes limites valem para uma loja cujos produtos você mesmo digitou — uma [loja vinculada ao catálogo](/integrations/telegram-mini-app#catalog-linked-storefronts) elimina os três primeiros ao puxar produtos, estoque e checkout da sua loja conectada.

- **Sem pagamentos próprios.** Os Order CTAs são handoffs de consulta, não Checkout API nem Stripe. (Uma loja vinculada ao catálogo repassa para o checkout da sua loja — veja [Checkout](/commerce/checkout).)
- **Sem rastreamento de estoque.** Os badges de disponibilidade nos product cards são rótulos de texto livre que você edita manualmente ("Esgotado", "Pré-venda", "Últimos 3"). (Uma loja vinculada ao catálogo lê o estoque real da sua loja.)
...
- **Uma loja, vários canais**: uma loja não fica mais presa 1:1 a uma Página. Crie-a uma vez e publique-a em uma página do Facebook *e* em um bot do Telegram ao mesmo tempo pela aba **Distribute** — as mesmas páginas, produtos e tema nos dois. Despublique um canal sem mexer no outro.
- **O preço é por organização, não por Página**: adicionar uma segunda página do Facebook NÃO muda o preço do seu plano.
- **Editor compartilhado com o Telegram**: o mesmo editor constrói a loja para cada canal em que ela é publicada. Quando uma loja está vinculada a uma Página e a um bot do Telegram, os dois compartilham o mesmo conteúdo — edite uma vez e publique nos dois.
