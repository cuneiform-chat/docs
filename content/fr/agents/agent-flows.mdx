...
<Callout type="info">
  Agent Forms fonctionne sur **WhatsApp, Telegram, Messenger et le widget de chat de votre site web**, et il est inclus pour tout le monde, essai gratuit compris. Concevez un flow une fois ; il s'affiche nativement sur chaque canal pris en charge.
</Callout>
...
Les flows se trouvent sous **Agent Forms** dans le panneau d'administration du client. Chaque flow comporte :
...
Vous n'êtes pas obligé de partir d'un éditeur vierge. Ouvrez l'onglet **Modèles** dans la liste Agent Forms pour parcourir une bibliothèque sélectionnée de flows prêts à l'emploi : captation de leads, réservation de démo, prise en charge du support, retours produit, et plus encore. Cliquez sur **Utiliser ce modèle** et un brouillon modifiable est créé dans votre propre espace de travail : les écrans, les champs, la logique et les traductions sont pré-remplis. Reliez le brouillon à un numéro WhatsApp, ajustez ce que vous voulez, puis publiez.

Un modèle n'est qu'un point de départ : une fois votre copie créée, c'est un flow normal qui vous appartient entièrement et que vous pouvez modifier librement. Toute la bibliothèque de modèles est ouverte à tout le monde.
...
Agent Forms lui-même est inclus pour tout le monde, essai gratuit compris : il n'y a rien à débloquer avant de créer votre premier flow. Ce que votre forfait détermine, c'est le nombre de flows qui peuvent être **publiés en même temps** et le nombre de langues dans lesquelles chaque flow publié paraît.

| Forfait | Flows publiés | Langues par flow |
|------|-----------------|------------------|
| Trial | 1 | 1 |
| Starter | 1 | 1 |
...
Comment le décompte s'applique :

- **Seuls les flows publiés comptent.** Les brouillons sont illimités — créez-en autant que vous voulez, publiez celui que vous voulez en ligne.
- **Republier est gratuit.** Mettre à jour un flow déjà publié ne consomme pas une place de plus.
- **Dépubliez pour permuter.** À la limite, dépubliez un flow et la place se libère aussitôt pour un autre.
- **Le questionnaire [Product Finder](/commerce/product-tools#product-finder) ne compte pas.** Les flows que la plateforme gère pour vous restent en dehors de votre propre quota.

Les 23 types de composants sont disponibles pour tout le monde — aucun composant n'est réservé — et la génération de formulaires par IA est ouverte à tous.
