...
Les flows se publient par langue. Traduisez le flow une fois, choisissez les langues dans lesquelles vous publiez (la plateforme prend en charge `en`, `es`, `pt-BR`, `fr`, `ru`, `ar` aujourd'hui) et chaque client voit la version qui correspond à la langue de son WhatsApp. Si la langue d'un client n'est pas publiée, la langue par défaut de l'intégration est utilisée comme repli.
