...
- **Vos clients ne quittent jamais Messenger.** Pas d'installation d'app, pas de domaine à retenir et pas de page de checkout séparée à construire. La boutique s'ouvre dans la WebView de Messenger et hérite du thème et de la langue de l'utilisateur.
...
- **Gratuit sur tous les plans.** Aucun plafond sur les pages, les blocs par page ou les docs produit — sur aucun plan, essai gratuit compris. La tarification est par organisation — ajouter une deuxième Page Facebook ne change pas votre prix.
...
- **Deux façons d'alimenter les produits.** Une boutique **construite à la main** est une surface catalogue + capture de leads soignée — vous saisissez les produits vous-même, et elle ne fait ni paiements, ni suivi des stocks, ni livraison. Une boutique **reliée au catalogue** tire ses produits d'une [boutique Shopify ou WooCommerce connectée](/commerce) et transfère les acheteurs vers votre vrai checkout.
...
<Callout type="info">
  **Deux façons de démarrer une boutique.** La plus rapide est le hub **Storefront** (menu latéral → **Storefront** → **Créer une boutique**) : il crée une boutique vierge que vous pouvez d'abord construire, puis publier sur n'importe quel canal — une Page Facebook, un bot Telegram ou les deux — depuis son onglet **Distribute**. L'entrée par Page ci-dessous (**Intégrations → votre Page Facebook → Configurer la mini app**) fonctionne toujours et reste pratique quand vous êtes déjà sur la carte de la Page.
</Callout>
...
La même portée que la Boutique Telegram. Ces limites s'appliquent à une boutique dont vous avez saisi les produits vous-même — une [boutique reliée au catalogue](/integrations/telegram-mini-app#catalog-linked-storefronts) lève les trois premières en tirant ses produits, son stock et son checkout de votre boutique connectée.

- **Pas de paiements en propre.** Les Order CTA sont des transferts de demande, pas des Checkout API ni Stripe. (Une boutique reliée au catalogue transfère vers le checkout de votre boutique — voir [Checkout](/commerce/checkout).)
- **Pas de suivi des stocks.** Les badges de disponibilité sur les product cards sont des libellés texte libre que vous éditez manuellement (« Sold out », « Pre-order », « Last 3 »). (Une boutique reliée au catalogue lit le vrai stock de votre boutique.)
