...
L'URL de la boutique a la forme `https://tele-mini.cuneiform.chat/{your-org-slug}` (avec `?i={integration_id}` ajouté seulement si votre organisation a plus d'un bot Telegram). Copiez cette URL depuis la fiche d'intégration pour des QR codes, des publicités, ou pour enregistrer un lien de lancement direct BotFather `t.me/<bot>/<app>` (voir le playbook BotFather sur la fiche du bot).
