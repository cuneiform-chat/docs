...
Cancellations are handled through the Polar customer portal. Contact support, or — if your subscription has entered a payment-failed state — use the **Update Payment** button on the **Your Plan** card to open the portal and cancel from there. Access continues until the end of your current billing period. On a billing term longer than a month that means the end of the term you have already paid for, and the remaining time is not refunded.
...
Your subscription is billed on the cadence you chose at checkout. Which cadences are currently offered, and what each costs, is listed on the [pricing page](https://cuneiform.chat/pricing) — that page reads the live catalog, so it is always the authoritative answer.

Whichever cadence you are on, Platform Credits are re-granted on their own cycle of roughly 30 days. A subscription billed once a year is therefore charged annually but still receives a fresh credit allowance about every 30 days. See [Usage & Credits](/billing/usage).
