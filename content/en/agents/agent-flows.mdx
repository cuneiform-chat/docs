...
<Callout type="info">
  Agent Forms runs on **WhatsApp, Telegram, Messenger, and your website chat widget**, and it's included for everyone — free trial included. Build a flow once; it renders natively on every supported channel.
</Callout>
...
|---------|-------------------|-------|
| **WhatsApp** | The agent sends a native WhatsApp Flow button; the form opens inside WhatsApp | Publishes per-language to Meta; the only channel with native rich-text limits set by WhatsApp |
...
Flows live under **Agent Forms** in the customer admin panel. Each flow has:
...
The composer offers **AI form generation** to everyone: write a one-line prompt ("a hotel booking form with date-range and photo upload"), or upload a PDF / image of an existing form, and the assistant drafts the flow for you. The draft is fully editable — every field is round-trippable in the composer (no opaque AI snapshots).
...
Agent Forms itself is included for everyone, free trial included — there's nothing to unlock before you build your first flow. What your plan governs is how many flows can be **live at once**, and how many languages each live flow is published in.

| Plan | Published flows | Locales per flow |
|------|-----------------|------------------|
| Trial | 1 | 1 |
| Starter | 1 | 1 |
...
- **Only live flows count.** Drafts are unlimited — build as many as you like, publish the one you want live.
- **Republishing is free.** Updating a flow that's already published doesn't consume another slot.
- **Unpublish to swap.** At the limit, unpublish one flow and the slot is immediately free for another.
- **The [Product Finder](/commerce/product-tools#product-finder) quiz doesn't count against you.** Flows the platform manages on your behalf sit outside your own allowance.

All 24 component types are available on every plan — no component is held back — and AI form generation is open to everyone.
