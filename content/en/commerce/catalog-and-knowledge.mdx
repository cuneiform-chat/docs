...
Recommendations are drawn from the same synced catalog, and are **restricted to products that are actually in stock** — the agent will not build a shopper's enthusiasm around something they can't buy. They work across both connected stores.
...
Deletions arrive by webhook like any other change. If one is missed, the weekly reconcile clears it — that pass runs once a week, not nightly, so a missed deletion can linger for a few days. To clear it now, run **Re-sync now** and then **Rebuild** product knowledge.
