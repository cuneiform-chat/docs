...
- **Live inventory** — prices and stock read fresh at answer time
...
### Your catalog — press "Resync now"

Nothing is imported at connect. A background job reconciles catalogs daily, but its catch-up pass only runs for stores that have synced at least once before — a store that has never synced has nothing to catch up *from*. In practice that means a manually-connected store's catalog is fully populated by the **weekly sweep**, which can be up to seven days away.

**Don't wait for it.** Open **Commerce → WooCommerce → Catalog** and click **Resync now**. That runs the same import the guided flow would have run at connect, and it finishes in minutes.
...
- **A read-only store never recovers.** WooCommerce rejects every registration attempt from a read-only key. The job retries every hour, is rejected every hour, and logs nothing you'll see. It will keep failing indefinitely.

The symptom of a read-only store is not an error message — it's that catalog, stock, and order changes only ever show up on the next scheduled sync instead of arriving as they happen.
...
Prices and stock are read fresh at answer time, so recommendations stay limited to what's actually available. WooCommerce sends stock changes along with product updates rather than as separate inventory events, so a stock-only change lands with the next product update or the next sync. See [Catalog & Knowledge](/commerce/catalog-and-knowledge).

### Opt-in order-status updates

After a shopper verifies their order in chat, they can opt in to shipping updates on a supported channel. Updates require two things: that you enabled them on the WooCommerce connection, and that the shopper opted in. See [Orders](/commerce/orders#order-status-updates).
...
If you connected manually, this is expected — nothing was imported. Click **Resync now** on the Catalog tab.
...
Both gates have to be open: enable notifications on the WooCommerce connection, and have the shopper opt in in chat on a supported channel.
...
If you still have a **Shopify** store connected, the workspace-wide steps are skipped — disconnecting one store never tears down the other store's commerce setup.
...
Because WooCommerce refuses to register webhooks for a read-only key, and without webhooks nothing updates in real time. The integration still never changes your catalog, your stock, or your existing orders. The only thing it ever creates is a pending order for a shopper checking out from a chat — see [Checkout](/commerce/checkout).
