...
- [Document attachments](/integrations/document-attachments) — upload a file and ask about it (on by default, paid plans)
...
Someone can upload a file in a DM or channel and ask your agent about it. Only files **uploaded to Slack** are read — a file merely linked from Google Drive, Dropbox or another service isn't fetched. Several files in one message are all read.

This is **on by default** on Slack — an admin can switch **Allow document attachments** off on the integration card. See [Document Attachments](/integrations/document-attachments) for supported types, limits and setup.
