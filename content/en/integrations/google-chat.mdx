...
- Text-based responses with conversation memory and thread context awareness
...
**Thread context awareness (Spaces):** When you @mention the bot in a Space thread, it reads the full thread history first — understanding what your team was already discussing — before responding. Follow-up @mentions in the same thread maintain that conversational continuity, so the bot stays in sync with the thread without you needing to repeat context.
