...
- [Document attachments](/integrations/document-attachments) — send a file and ask about it (on by default, paid plans)
...
This is **on by default** on Telegram — an admin can switch **Allow document attachments** off on the integration card. Images or scans still need **Premium parsing for complex documents**, which stays off until an admin turns it on. See [Document Attachments](/integrations/document-attachments) for supported types, limits and setup.
