...
- [Document attachments](/integrations/document-attachments) — attach a file and ask about it (on by default, paid plans)
...
Someone can attach a file to a message and ask your agent about it. **Every file attached to a message is read**, not just the first. A message carrying only a sticker has nothing to read, so the bot replies as it does today.

This is **on by default** on Discord — an admin can switch **Allow document attachments** off on the integration card. See [Document Attachments](/integrations/document-attachments) for supported types, limits and setup.
...
| **Developer Portal** | Opens Discord Developer Portal for this app |
