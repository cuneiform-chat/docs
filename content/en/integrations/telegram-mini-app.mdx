...
- **Customers never leave Telegram.** No app install, no domain to remember, and no separate checkout page to build. The storefront opens in Telegram's WebView and inherits the user's theme, language, and login.
...
- **Free on every plan.** No caps on pages, blocks per page, or product docs — on any plan, free trial included. Pricing is per-org — adding a second Telegram bot doesn't change your price.
...
- **Two ways to source products.** A **hand-built** storefront is a polished catalog + lead-capture surface — you type the products in yourself, and it does no payments, no inventory tracking, and no shipping; pair it with a human in your inbox and you have a complete sales loop. A **catalog-linked** storefront draws its products from a connected Shopify or WooCommerce store and hands shoppers to your real checkout. [See below](#catalog-linked-storefronts).
...
<Callout type="info">
  **Two ways to start a storefront.** The fastest is the **Storefront** hub (left nav → **Storefront** → **Create storefront**): it mints a fresh storefront you can build first and publish to any channel later — to a Telegram bot, a Facebook Page, or both — from its **Distribute** tab. The per-bot entry below (**Integrations → your bot → Configure mini app**) still works and is handy when you're already on the bot's card.
</Callout>
...
There is no cap on the total number of product docs you can attach (see [Tier limits](#tier-limits)).

## Order inquiries & cart (optional) [#orders-and-cart]

You can attach an **Order CTA** to any product card or product grid. On a **hand-built** storefront this is purely a structured-message handoff — **no payments, no inventory, no fulfillment** — and it lands in your inbox for a human to pick up. On a **catalog-linked** storefront the same CTA can hand the shopper to your store's real checkout instead; see [Catalog-linked storefronts](#catalog-linked-storefronts).
...
## Catalog-linked storefronts [#catalog-linked-storefronts]

Everything above describes a storefront you build by hand: you type in the products, prices, and availability, and orders arrive as inquiries. That's still the right shape for a brochure, a service catalog, or a menu.

If you have a **[Shopify or WooCommerce store connected](/commerce)**, a storefront can instead be **linked to that catalog**. Its product blocks are drawn from your real catalog, prices and stock come from the store rather than from something you edit by hand, and its order CTAs can hand a ready-to-buy shopper to your store's own checkout with their cart intact — see [Checkout](/commerce/checkout).

| | Hand-built | Catalog-linked |
|---|---|---|
| **Products** | You type them in | Pulled from your connected store |
| **Price & availability** | Free-text fields you maintain | Read from your store |
| **Order CTA** | Inquiry handoff to your Inbox | Handoff to your store's checkout |
| **Needs a connected store** | No | Yes |

A storefront is scoped to one source, and once it has product blocks on it that scope is fixed — so a hand-built storefront can't quietly start behaving like a catalog-linked one, or the reverse. Storefronts created before catalog linking existed are unscoped and keep working exactly as they always did.
...
Available on every plan, including the free trial — and there are **no caps** on pages, blocks per page, or product docs on any plan, the free trial included.

Notes:

- A product grid or carousel **counts as 1 block** in the composer's block list, no matter how many items or slides it contains.
- **Pricing is per-org, not per-bot.** Adding a second (or fifth) Telegram bot to your org does **not** change your price. Each bot gets its own storefront inside the same plan.
...
These limits apply to a storefront whose products you typed in yourself. A [catalog-linked storefront](#catalog-linked-storefronts) lifts the first three by getting its products — and its checkout — from your connected store.

- **No payments of its own.** Order CTAs are inquiry handoffs, not Stripe / WhatsApp Pay / Telegram Stars checkouts. (A catalog-linked storefront hands off to your store's checkout instead of taking payment here.)
- **No inventory tracking.** Availability badges are free-text labels you edit manually. (A catalog-linked storefront reads real stock from your store.)
- **No shipping calculations / address validation.** The delivery address is collected as free-form text. (Shipping on a catalog-linked storefront is your store's own, calculated at its checkout.)
