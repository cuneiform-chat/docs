...
## Generate a storefront with AI

Don't want to start from a blank canvas? Describe your storefront in plain language and let the AI draft it for you.

Click **Generate with AI** in the composer, then either:
...
<Callout type="info">
  **Free on every plan.** AI generation is available to anyone who can open the composer — there's no separate add-on or higher tier required. (You still need the **Integration manage** permission, same as any other composer edit.)
</Callout>

A few things to know:

- **It's a starting point, not a finished store.** The AI drafts text and structure; **it does not generate images** — every image slot comes back empty for you to fill from the inline uploader. Pricing, product names, and descriptions are best-guesses from your prompt — edit them to match reality.
- **English first.** Generation produces English content. Use the composer's **Translate** flow afterward to fill in other languages.
- **It replaces your current draft.** If the composer already has unsaved blocks, generating will ask you to confirm before overwriting them.
- **One shot.** Each generation produces a fresh draft from your prompt — there's no back-and-forth refine chat. To try a different direction, just generate again with a new prompt.
...
- **`cart` mode** — customer taps "Add to cart" → product is added to a local cart (lives in their browser only, keyed to your integration) → they review on the cart page (a **Cart review** block) and check out → one multi-line handoff lands in your inbox.
