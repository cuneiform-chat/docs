...
<Callout type="info">
  Esta página cubre la Mini App de tienda orientada al cliente que puedes publicar en cualquier integración de **Meta (Facebook)**. El motor de la tienda se comparte con la [Tienda de Telegram](/integrations/telegram-mini-app) — mismo compositor, mismos bloques, mismos temas — y una misma tienda puede publicarse en una página de Facebook *y* en un bot de Telegram a la vez. Solo el lanzador y unos pocos comportamientos específicos de Messenger difieren.
</Callout>
...
- **Los clientes nunca salen de Messenger.** Sin instalación de app, sin dominio que recordar y sin página de checkout aparte que construir. La tienda se abre en el WebView de Messenger y hereda el tema y el idioma del usuario.
...
- **Gratis en todos los planes.** Sin topes de páginas, bloques por página ni documentos de producto — en ningún plan, incluida la prueba gratuita. El precio es por organización — agregar una segunda página de Facebook no cambia tu precio.
...
- **Dos formas de obtener los productos.** Una tienda **construida a mano** es una superficie pulida de catálogo + captura de leads — tú escribes los productos, y no hace pagos, ni seguimiento de inventario, ni envíos. Una tienda **vinculada al catálogo** saca sus productos de una [tienda Shopify o WooCommerce conectada](/commerce) y lleva a los compradores a tu checkout real.
...
<Callout type="info">
  **Dos formas de empezar una tienda.** La más rápida es el hub **Storefront** (menú lateral → **Storefront** → **Crear tienda**): crea una tienda nueva que puedes construir primero y publicar en cualquier canal después — en una Página de Facebook, un bot de Telegram o ambos — desde su pestaña **Distribute**. La entrada por Página de abajo (**Integrations → tu Facebook Page → Configure mini app**) sigue funcionando y es práctica cuando ya estás en la tarjeta de la Página.
</Callout>
...
El mismo alcance que la Tienda de Telegram. Estos límites se aplican a una tienda cuyos productos escribiste tú — una [tienda vinculada al catálogo](/integrations/telegram-mini-app#catalog-linked-storefronts) levanta los tres primeros al obtener sus productos, su stock y su checkout de tu tienda conectada.

- **Sin pagos propios.** Las CTAs de pedido son transferencias de consulta, no Checkout API ni Stripe. (Una tienda vinculada al catálogo transfiere al checkout de tu tienda — consulta [Checkout](/commerce/checkout).)
- **Sin seguimiento de inventario.** Las insignias de disponibilidad en las product cards son etiquetas de texto libre que editas manualmente ("Agotado", "Pre-venta", "Quedan 3"). (Una tienda vinculada al catálogo lee el stock real de tu tienda.)
...
- **Una tienda, varios canales**: una tienda ya no está atada 1:1 a una Página. Créala una vez y publícala en una página de Facebook *y* en un bot de Telegram a la vez desde la pestaña **Distribute** — las mismas páginas, productos y tema en ambos. Despublica un canal sin tocar el otro.
- **El precio es por organización, no por Página**: agregar una segunda página de Facebook NO cambia el precio de tu plan.
- **Compositor compartido con Telegram**: el mismo compositor construye la tienda para cada canal en el que se publica. Cuando una tienda está vinculada a una Página y a un bot de Telegram, ambos comparten el mismo contenido — edítalo una vez y publícalo en los dos.
