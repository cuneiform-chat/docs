...
- **Customer-রা কখনো Telegram ছাড়ে না।** কোনো app install নেই, মনে রাখার মতো কোনো domain নেই, নিজে বানানোর মতো কোনো আলাদা checkout পাতা নেই। storefront Telegram-এর WebView-তে খোলে এবং user-এর theme, ভাষা ও login উত্তরাধিকার সূত্রে পায়।
...
- **প্রতিটি plan-এ free।** page, প্রতি page-এ block বা product doc-এ কোনো cap নেই — কোনো plan-এই নয়, free trial সহ। Pricing per-org — একটি দ্বিতীয় Telegram bot যোগ করলে আপনার price পরিবর্তন হয় না।
...
- **Product পাওয়ার দুটি উপায়।** **হাতে সাজানো** একটি storefront একটি পরিপাটি catalog + lead-capture surface: product আপনি নিজে লেখেন, আর সেটি কোনো payment, inventory বা shipping করে না; আপনার inbox-এ একজন মানুষের সাথে এটিকে যুক্ত করুন এবং আপনার একটি সম্পূর্ণ sales loop হয়ে যাবে। **Catalog-এর সাথে জোড়া** একটি storefront আপনার যুক্ত Shopify বা WooCommerce store থেকে product নেয় এবং ক্রেতাদের আপনার আসল checkout-এ পাঠায়। [নিচে দেখুন](#catalog-linked-storefronts)।
...
আপনি যত product doc attach করতে পারবেন তার মোট সংখ্যায় কোনো cap নেই ([Tier limits](#tier-limits) দেখুন)।

## Order inquiry ও cart (ঐচ্ছিক) [#orders-and-cart]

আপনি যেকোনো product card বা product grid-এ একটি **Order CTA** attach করতে পারেন। **হাতে সাজানো** একটি storefront-এ এটি সম্পূর্ণভাবে একটি structured-message handoff — **কোনো payment নেই, কোনো inventory নেই, কোনো fulfillment নেই** — আর সেটি একজন মানুষের তুলে নেওয়ার জন্য আপনার inbox-এ আসে। **Catalog-এর সাথে জোড়া** একটি storefront-এ সেই একই CTA বদলে ক্রেতাকে আপনার store-এর আসল checkout-এ নিয়ে যেতে পারে; দেখুন [Catalog-এর সাথে জোড়া storefront](#catalog-linked-storefronts)।
...
## Catalog-এর সাথে জোড়া storefront [#catalog-linked-storefronts]

উপরে বর্ণিত সবকিছু সেই storefront-এর জন্য যেটি আপনি হাতে সাজান: product, দাম ও প্রাপ্যতা আপনি নিজে লেখেন, আর order অনুরোধ হিসেবে আসে। একটি brochure, একটি পরিষেবার তালিকা বা একটি মেনুর জন্য এটিই এখনো সঠিক রূপ।

আপনার যদি একটি **[যুক্ত Shopify বা WooCommerce store](/commerce)** থাকে, তাহলে storefront-টি বদলে **সেই catalog-এর সাথে জোড়া** দেওয়া যায়। তার product block আপনার আসল catalog থেকে আসে, দাম ও stock আপনার হাতে লেখা কিছু নয় বরং store থেকে আসে, আর তার order CTA কিনতে প্রস্তুত একজন ক্রেতাকে cart অক্ষত রেখে আপনার নিজের store-এর checkout-এ নিয়ে যেতে পারে — দেখুন [Checkout](/commerce/checkout)।

| | হাতে সাজানো | Catalog-এর সাথে জোড়া |
|---|---|---|
| **Product** | আপনি লেখেন | যুক্ত store থেকে আসে |
| **দাম ও প্রাপ্যতা** | আপনার রক্ষণাবেক্ষণ করা free-text ঘর | আপনার store থেকে পড়া হয় |
| **Order CTA** | আপনার Inbox-এ inquiry handoff | আপনার store-এর checkout-এ handoff |
| **যুক্ত store লাগে** | না | হ্যাঁ |

একটি storefront একটি উৎসেই বাঁধা, আর তার উপর product block বসে গেলে সেই বাঁধন স্থির হয়ে যায় — তাই হাতে সাজানো একটি storefront নীরবে catalog-জোড়া storefront-এর মতো আচরণ করতে শুরু করতে পারে না, আর উল্টোটাও নয়। catalog-জোড়া আসার আগে তৈরি হওয়া storefront-গুলো কোনো উৎসে বাঁধা নয় এবং সেগুলো ঠিক আগের মতোই কাজ করে যায়।
...
## Tier limit [#tier-limits]

প্রতিটি plan-এ available, free trial সহ — এবং কোনো plan-এ page, প্রতি page-এ block বা product doc-এর **কোনো cap নেই**, free trial সহ।

নোট:

- একটি product grid বা carousel composer-এর block তালিকায় **১টি block হিসেবে গণ্য**, এতে যত item বা slide-ই থাকুক না কেন।
- **Pricing per-org, per-bot নয়।** আপনার org-এ একটি দ্বিতীয় (বা পঞ্চম) Telegram bot যোগ করলে আপনার price পরিবর্তন **হয় না**। প্রতিটি bot একই plan-এর ভেতরে নিজস্ব storefront পায়।
...
## হাতে সাজানো storefront যা করে না

এই সীমাবদ্ধতাগুলো সেই storefront-এর জন্য যাতে product আপনি নিজে লিখেছেন। [Catalog-এর সাথে জোড়া storefront](#catalog-linked-storefronts) প্রথম তিনটি তুলে দেয়, কারণ সে product — এবং checkout — আপনার যুক্ত store থেকেই নেয়।

- **নিজস্ব কোনো payment নেই।** Order CTA inquiry handoff, Stripe / WhatsApp Pay / Telegram Stars checkout নয়। (Catalog-জোড়া storefront বদলে আপনার store-এর checkout-এ handoff করে।)
- **কোনো inventory tracking নেই।** Availability badge হলো free-text label যা আপনি manually edit করেন। (Catalog-জোড়া storefront আপনার store থেকে আসল stock পড়ে।)
- **কোনো shipping calculation / address validation নেই।** Delivery address free-form text হিসেবে সংগ্রহ করা হয়। (Catalog-জোড়া storefront-এ shipping আপনার নিজের store-এরই shipping, তার checkout-এ হিসাব করা।)
