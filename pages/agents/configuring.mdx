...
Set which language your agent responds in:

- **Inherit from organization** (default) — Uses the organization's default language from Settings
- **Specific language** — Choose from 24 supported languages (Arabic, French, German, Spanish, Turkish, Chinese, Japanese, and more)

When set to a non-English language, the agent responds in that language. Safety and accuracy rules remain in English internally.
...
3. Configure the messages:
   ...
   - **Max Wait Time** — Auto-expire if no agent accepts (default: 30 minutes)
