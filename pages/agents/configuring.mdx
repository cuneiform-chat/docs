...
<Callout type="tip">
  Enable document references for use cases requiring transparency, such as legal, compliance, or internal knowledge bases where users need to verify sources.
</Callout>
...
When a user asks to speak with a human, the AI creates a handoff request visible on the **Live Chat** page. A team member accepts, chats with the user on their channel, and resolves when done.
