...
| Field | Description |
|-------|-------------|
| **Greeting Message** | First message shown when users start a conversation |
| **Fallback Response** | Message shown when the agent can't find an answer |

These are optional — leave blank to use default behavior.

<Callout type="tip">
  Set a helpful fallback response that provides alternatives, like contact information or suggested topics, when the agent can't answer.
</Callout>
