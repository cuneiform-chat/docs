...
Configure which language your agent responds in. You can set this at two levels:

- **Organization default** — Set in Account Management under Settings. All agents inherit this unless overridden.
- **Per-agent override** — Set in the agent's configuration. Choose a specific language or "Inherit from organization."

24 languages are supported including English, Arabic, French, German, Spanish, Turkish, Chinese, Japanese, Korean, Portuguese, and more.

When configured to a non-English language, the agent responds in that language while its accuracy and safety rules remain in English internally.

**Language resolution order:** channel override → agent language → organization default → English
...
6. **Set the language** — Use the Response Language setting to control which language the agent replies in, rather than adding language instructions to the persona text
